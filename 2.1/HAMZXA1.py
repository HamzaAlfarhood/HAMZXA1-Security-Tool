...
#           Version 2.1 (The Complete & Unified Edition) - By Hamza Alfarhood
# =============================================================================
...
# =============================
CURRENT_VERSION = "2.1"
VERSION_URL = "https://raw.githubusercontent.com/HamzaAlfarhood/HAMZXA1-Security-Tool/refs/heads/main/Version"
UPDATE_PAGE_URL = "https://github.com/HamzaAlfarhood/HAMZXA1-Security-Tool"

LOG_FILE = Path("HAMZXA1_log.txt")
SYSTEM32_DIR = Path(r"C:\Windows\System32")
WSVCZ_TARGET_DIR = Path(r"C:\Windows\System32\wsvcz")  # v1.5 legacy rootkit folder
...
#      (Merged from v1.5 + v2.0)
# =============================
# Files known to be planted by the rootkit directly in System32
KNOWN_THREAT_FILENAMES = [
    "svctrl64.exe",     # v2.0: Rootkit service controller
    "winring0x64.sys",  # v2.0: Kernel-level driver used by rootkit
    "wlogz.dat",        # v2.0: Rootkit data/config file
]
...
def resource_path(relative_path):
    try:
        base_path = sys._MEIPASS   # When packaged as EXE
    except Exception:
...
# =============================
#      Core Toolkit Functions (v2.0 Method)
# =============================
...
def remove_protection(path: Path, log_widget: tk.Text):
    ...
    log_message(f"Stripping protection from {path.name}...", log_widget)
    os.system(f'attrib -h -s -r "{path}" /S /D')

def kill_processes_holding_file(file_path: Path, log_widget: tk.Text):
    """
    Terminates processes using the target file (as EXE or loaded DLL).
    Runs up to 3 waves for resilience against self-respawning malware.
    """
    killed_count = 0
    target_path_str = str(file_path.resolve()).lower()
    log_message(f"Multi-wave hunt for processes holding: '{file_path.name}'...", log_widget)

    for attempt in range(3):
        killed_in_wave = 0
        for proc in psutil.process_iter(['pid', 'name', 'exe']):
            try:
                # Check if process IS the target executable
                if proc.info['exe'] and str(Path(proc.info['exe']).resolve()).lower() == target_path_str:
                    proc.kill()
                    killed_in_wave += 1
                    log_message(f"WAVE {attempt+1}: KILLED EXE process {proc.info['name']} (PID: {proc.info['pid']})", log_widget)
                    continue
                # Check if process has the target DLL loaded in memory
                ...
                log_message(f"Process check error (PID {proc.info['pid']}): {e}", log_widget)
                continue

        if killed_in_wave > 0:
            killed_count += killed_in_wave
            log_message(f"Wave {attempt+1}: Eliminated {killed_in_wave} process(es).", log_widget)
            time.sleep(0.5)
        else:
            log_message(f"Wave {attempt+1}: No processes holding the file.", log_widget)
            break

    if killed_count > 0:
        log_message(f"Total eliminated: {killed_count} process(es).", log_widget)

# =============================
#      5-Stage Annihilator (v2.0 Method)
# =============================
def find_and_destroy_malicious_service(threat_name: str, log_widget: tk.Text):
    log_message(f"Hunting service responsible for {threat_name}...", log_widget)
    try:
...
def destroy_malicious_scheduled_tasks(threat_name: str, log_widget: tk.Text):
    log_message(f"Hunting scheduled tasks for {threat_name}...", log_widget)
    task_file = SYSTEM32_DIR / "Tasks" / threat_name.replace('.exe', '')
    if task_file.exists():
        log_message(f"Found task definition file: {task_file}", log_widget)
        take_ownership(task_file, log_widget)
        remove_protection(task_file, log_widget)
        try:
            os.remove(task_file)
            log_message("SUCCESS: Deleted task definition file.", log_widget)
        except Exception as e:
            log_message(f"Failed to delete task file: {e}", log_widget)
    try:
        ...
        if "ERROR:" not in result:
            ...
            log_message(f"SUCCESS: Task '{task_query_name}' destroyed.", log_widget)
        else:
            log_message("No active scheduled task found.", log_widget)
    except Exception as e:
        log_message(f"Scheduled task hunt error: {e}", log_widget)

def clean_all_persistence(threat_name: str, log_widget: tk.Text):
    log_message(f"Deep registry cleanup for '{threat_name}'...", log_widget)
    persistence_keys = [
        ...
        for key_path in persistence_keys:
            try:
                with winreg.OpenKey(hive, key_path, 0, winreg.KEY_ALL_ACCESS) as key:
                    ...
                    # IFEO: check Debugger value in subkeys
                            ...
                            while True:
                                ...
                                with winreg.OpenKey(key, sub_key_name, 0, winreg.KEY_ALL_ACCESS) as sub_key:
                                    try:
                                        ...
                                        log_message(f"IFEO subkey error '{sub_key_name}': {e}", log_widget)
                                j += 1
                        except OSError:
                            pass

                    # BHOs: check CLSID default value
                        ...
                        try:
                            ...
                            while True:
                                ...
                                with winreg.OpenKey(key, clsid_key_name, 0, winreg.KEY_ALL_ACCESS) as clsid_key:
                                    try:
                                        dll_path, _ = winreg.QueryValueEx(clsid_key, None)
                                        if isinstance(dll_path, str) and threat_name in dll_path:
                                            log_message(f"FOUND BHO: CLSID '{clsid_key_name}' -> '{dll_path}'", log_widget)
                                            winreg.DeleteKey(key, clsid_key_name)
                                            log_message("SUCCESS: Removed BHO CLSID key.", log_widget)
                                    except FileNotFoundError:
                                        pass
                                    except Exception as e:
                                        log_message(f"BHO subkey error '{clsid_key_name}': {e}", log_widget)
                                j += 1
                        except OSError:
                            pass
...
def annihilate_now(path: Path, log_widget: tk.Text):
    """5-Stage annihilation: Tasks → Service → Registry → Kill Process → Delete File."""
    log_message(f"--- INSTANT ANNIHILATION: {path.name} ---", log_widget)
    try:
        log_message("--- Stage 1: Destroying Scheduled Tasks ---", log_widget)
        destroy_malicious_scheduled_tasks(path.name, log_widget)

        log_message("--- Stage 2: Destroying Service ---", log_widget)
        find_and_destroy_malicious_service(path.name, log_widget)

        log_message("--- Stage 3: Deep Registry Cleanup ---", log_widget)
        clean_all_persistence(path.name, log_widget)

        log_message("--- Stage 4: Killing Live Processes & Loaded Modules ---", log_widget)
        kill_processes_holding_file(path, log_widget)

        log_message("--- Stage 5: Final File Deletion ---", log_widget)
        take_ownership(path, log_widget)
        remove_protection(path, log_widget)
        time.sleep(0.5)
        try:
            os.remove(path)
        except Exception as e:
...
# =============================
#      Scanner Logic
#      (v1.5 wsvcz folder + v2.0 System32 patterns, merged)
# =============================
def find_wsvcz_threats():
    """v1.5 Legacy: scans the dedicated wsvcz rootkit folder inside System32."""
    if not WSVCZ_TARGET_DIR.exists():
        return []
    found = [f for f in WSVCZ_TARGET_DIR.iterdir() if f.is_file()]
    return found

def find_system32_threats():
    """v2.0: scans System32 for polymorphic (uNNNN.dll/exe/dat) and known threat filenames."""
    threats = []
    ...
    for f in SYSTEM32_DIR.iterdir():
        name = f.name.lower()
        is_polymorphic = (
            name.startswith("u") and
            (name.endswith(".dll") or name.endswith(".exe") or name.endswith(".dat")) and
            name[1:-4].isdigit()
        )
        is_known = name in KNOWN_THREAT_FILENAMES
        if f.is_file() and (is_polymorphic or is_known):
            threats.append(f)
    return threats

def find_all_threats():
    """Combined scanner: wsvcz folder (v1.5) + System32 patterns (v2.0)."""
    threats = find_wsvcz_threats() + find_system32_threats()
...
def clean_usb_drive(root: Path, log_widget: tk.Text):
    ...
    try:
        ...
        if not any([sysvolume_dir, shortcut_files, data_dir]):
            log_message(f"No known USB virus patterns found on {root}.", log_widget)
            return
        ...
        if data_dir:
            log_message(f"Restoring user files from: {data_dir.name}", log_widget)
...
class HAMZXA1_App(tk.Tk):
    def __init__(self):
        ...
        # Icon setup (PNG for taskbar/Alt+Tab, ICO for title bar)
        try:
        ...
        # --- Admin Check: notify and exit if not admin ---
        if not is_admin():
            self.after(100, self._show_admin_error)
            return

        self.create_main_layout()

    def _show_admin_error(self):
        messagebox.showerror(
            "⛔ مطلوب صلاحيات المسؤول",
            "هذا البرنامج يتطلب تشغيله كـ Administrator.\n\n"
            "الرجاء إغلاق البرنامج والنقر بزر الماوس الأيمن عليه ثم اختيار:\n"
    ...
    def create_main_layout(self):
        ...
        tk.Button(header_frame, text="Check for Updates", font=("Segoe UI", 9), bg=COLOR_GRAY, fg="white", relief="flat",
                  command=lambda: check_for_updates(self.log_widget)).pack(side="right", pady=(10, 0))
        ...
        self.create_action_card(system_card, "System Scanner",
                                "Finds and destroys polymorphic rootkits (wsvcz + System32) and all persistence mechanisms.",
        ...
        self.create_action_card(usb_card, "USB Cleaner",
                                "Cleans shortcut viruses and restores hidden files on removable drives.",
                                "💾", self.show_usb_selection)

        # Log area
        log_frame = tk.Frame(self, bg=COLOR_CARD, height=250)
    ...
    def create_action_card(self, parent, title, desc, icon_text, command):
        ...
        tk.Button(content, text=f"Launch {title}", font=("Segoe UI Semibold", 11), bg=COLOR_ACCENT, fg="black",
                  relief="flat", padx=20, pady=8, command=command).pack(pady=(20, 0))
    ...
    def show_popup_window(self, title):
        ...
        tk.Button(header, text="← Back to Main", font=("Segoe UI", 10), bg=COLOR_GRAY, fg="white",
                  relief="flat", command=popup.destroy).pack(side="left")
    ...
    def start_system_scan(self):
        ...
        threats = find_all_threats()
        popup, frame = self.show_popup_window("System Scanner")

        if not threats:
            log_message("Scan complete. No threats found.", self.log_widget)
            tk.Label(frame, text="✅ System Clean", font=("Segoe UI Semibold", 24), fg=COLOR_SUCCESS, bg=COLOR_BG).pack(pady=50)
            tk.Label(frame, text="No threats detected.", font=("Segoe UI", 11), fg=COLOR_TEXT, bg=COLOR_BG).pack()
            return

        log_message(f"Found {len(threats)} threat(s). Awaiting action.", self.log_widget)
        tk.Label(frame, text="⚠️ Threats Detected", font=("Segoe UI Semibold", 22), fg=COLOR_WARNING, bg=COLOR_BG).pack(anchor="w")
        ...
        for f in threats:
            ...
            # Show source tag for clarity
            source_tag = "[wsvcz]" if WSVCZ_TARGET_DIR in f.parents else "[System32]"
            tk.Label(row, text=f"{source_tag}  {f.name}", font=("Consolas", 10), fg=COLOR_TEXT_BRIGHT, bg=COLOR_CARD).pack(side="left", padx=6)
        ...
        def on_annihilate():
            ...
            if not targets:
                return
            if not messagebox.askyesno("Confirm Annihilation",
                    f"5-STAGE ATTACK will destroy the rootkit infrastructure\nand delete {len(targets)} file(s) INSTANTLY.\n\nProceed?"):
                return
            ...
            def worker():
                ...
                self.after(100, lambda: (
                    popup.destroy(),
                    messagebox.showinfo("Complete", "Annihilation finished.\nPlease restart your computer as a precaution.")
                ))
            threading.Thread(target=worker, daemon=True).start()

        buttons = tk.Frame(frame, bg=COLOR_BG)
        buttons.pack(fill="x", pady=(15, 0))
        annihilate_button = tk.Button(buttons, text="💥 Annihilate Now", font=("Segoe UI Semibold", 12),
                                       bg=COLOR_ERROR, fg="white", relief="flat", padx=20, pady=8, command=on_annihilate)
    ...
    def show_usb_selection(self):
        ...
        if not drives:
            log_message("No removable drives found.", self.log_widget)
            tk.Label(frame, text="✅ No USB Drives Found", font=("Segoe UI Semibold", 24), fg=COLOR_SUCCESS, bg=COLOR_BG).pack(pady=50)
            return

        log_message(f"Found {len(drives)} USB drive(s).", self.log_widget)
        tk.Label(frame, text="Select Drives to Clean", font=("Segoe UI Semibold", 22), fg=COLOR_ACCENT, bg=COLOR_BG).pack(anchor="w")
        check_vars = []
        for drive in drives:
            v = tk.BooleanVar(value=True)
            tk.Checkbutton(frame, text=str(drive), variable=v, font=("Segoe UI", 11),
                           bg=COLOR_BG, fg=COLOR_TEXT, selectcolor=COLOR_CARD,
                           activebackground=COLOR_BG, activeforeground=COLOR_TEXT_BRIGHT).pack(anchor="w", padx=10, pady=5)
            check_vars.append((v, drive))

        def on_clean():
            targets = [d for var, d in check_vars if var.get()]
            if not targets or not messagebox.askyesno("Confirm Clean", f"Clean {len(targets)} selected drive(s)?"):
                return
        ...
        clean_button = tk.Button(frame, text="Clean Selected Drives", font=("Segoe UI Semibold", 11),
                                  bg=COLOR_SUCCESS, fg="white", relief="flat", padx=16, pady=6, command=on_clean)
